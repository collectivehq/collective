...
from apps.discussions.permissions import can_reorganise, can_resolve_discussion, can_shape_tree, can_view_drafts
...
class TestDiscussionSpacePermissions:
    def test_facilitator_can_shape_tree(self, open_space_with_users):
        space, creator, _, _ = open_space_with_users
        assert can_shape_tree(creator, space) is True

    def test_participant_cannot_shape_tree(self, open_space_with_users):
        space, _, participant, _ = open_space_with_users
