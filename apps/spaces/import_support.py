...
def _materialize_discussion(*, parent: Node, space: Space, author: User, discussion: ImportedDiscussion) -> None:
    created = node_services.create_child_discussion(parent=parent, space=space, label=discussion.label)
    if discussion.content_html:
        node_services.create_post(discussion=created, author=author, content=discussion.content_html)
